...
// Flags blank fields, stale sections (generated for a previous target), missing
// map exhibits, and mismatched coordinates. Returns a structured issue list the
// SCIP Quality Auditor agent interprets. No data is ever modified.
...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`scipQualityAudit: ${rec.site_name || scip_record_id} → ${verdict} (${counts.critical}C/${counts.warning}W/${counts.info}I) for ${user.email}`);
