...
export default function TalonFit() {
  ...
  const sarfPacket = session?.sarfPacket || null;
  const zoningDecision = session?.zoningDecision || null;
  ...
  useEffect(() => {
    const lat = Number(zoningDecision?.coordinates?.lat ?? sarfPacket?.coordinates?.lat);
    const lon = Number(zoningDecision?.coordinates?.lng ?? sarfPacket?.coordinates?.lng);
    if (!Number.isFinite(lat) || !Number.isFinite(lon) || anchor) return;
