...
export default function HawkLawSessionDetail() {
  ...
  const handleSendToAttorney = () => {
    if (!attorneyEmail || !attorneyEmail.includes("@")) return;
    const triage = session?.triage_result || "";
    const triageLabel = { green: "✓ Low Risk", yellow: "⚠ Moderate Risk", red: "⛔ High Risk" }[triage] || triage;
    const reasons = (session?.triage_reasons || [])
      .map(r => (typeof r === "string" ? r : r?.concern || r?.reason || ""))
      .filter(Boolean)
      .map(r => `  • ${r}`)
      .join("\n");
    const clauseCount = reviewData?.clauses?.length || 0;
    const score = reviewData?.overall_score;
    const scoreLabel = score != null
      ? ["", "Very Landlord-Unfavorable", "Unfavorable", "Neutral", "Favorable", "Very Favorable"][Math.round(score)] || `${score}/5`
      : "Not yet run";

    const subject = encodeURIComponent(`Hawk Law Analysis — ${session?.file_name || "Lease Document"}`);
    const body = encodeURIComponent(
`HAWK LAW LEASE ANALYSIS REPORT
Generated by SiteHawk™ AI · ${new Date().toLocaleDateString()}
════════════════════════════════════════

Document: ${session?.file_name || "Lease Document"}
...
TRIAGE RESULT: ${triageLabel}
${session?.triage_summary || ""}

KEY CONCERNS:
${reasons || "  None flagged."}

FULL REVIEW:
Clauses Analyzed: ${clauseCount}
Overall Landlord Favorability: ${scoreLabel}

════════════════════════════════════════
DISCLAIMER: This analysis was generated by Hawk Law, an AI-powered lease
analysis tool built on Anthropic technology. This is NOT legal advice.
The recipient must consult a licensed attorney before executing any agreement.
SiteHawk Inc. assumes no liability for decisions made based on this report.
════════════════════════════════════════`
    );
    window.open(`mailto:${encodeURIComponent(attorneyEmail)}?subject=${subject}&body=${body}`, "_blank");
    setAttorneySent(true);
    setTimeout(() => {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex gap-2 shrink-0 flex-wrap">
          ...
          <Button size="sm" variant="outline" onClick={() => setShowAttorneyModal(true)}>
      ...
      {showAttorneyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl shadow-2xl p-7 max-w-sm w-full space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                <Send className="w-5 h-5 text-primary" />
              </div>
              <h3 className="font-heading font-bold text-lg text-foreground">Send to Attorney</h3>
            </div>
            <p className="text-sm text-muted-foreground">
              Enter your attorney's email address. A pre-formatted Hawk Law analysis
              report will open in your email client, ready to send.
            </p>
            <input
              type="email"
              value={attorneyEmail}
              ...
              className="w-full rounded-xl border border-border bg-secondary/20 px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
              onKeyDown={e => e.key === "Enter" && handleSendToAttorney()}
              autoFocus
            />
            <div className="flex gap-2">
              <Button
                onClick={handleSendToAttorney}
                disabled={!attorneyEmail || !attorneyEmail.includes("@") || attorneySent}
                className="flex-1"
              >
                {attorneySent ? "✓ Opening email client…" : "Open in Email →"}
              </Button>
              <Button
                variant="outline"
                onClick={() => { setShowAttorneyModal(false); setAttorneyEmail(""); setAttorneySent(false); }}
              >
                Cancel
              </Button>
            </div>
            <p className="text-xs text-muted-foreground">
              Opens your default email client with the full analysis pre-filled.
              No data leaves SiteHawk servers — the email is composed on your device.
            </p>
          </div>
        </div>
      )}
