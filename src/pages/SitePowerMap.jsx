 ...
 *
 * Purpose: give the contractor an at-a-glance picture of the power grid around
 * a SCIP candidate site. Everything is driven by the Target A coordinate (the
 * canonical SCIP active target) until the user switches to Target B or C.
 *
 * For each target it queries LIVE (no CSV):
 *   • HIFLD transmission lines within ~2 mi  (hifldTransmissionLines fn)
 *   • HIFLD substations within ~10 mi        (Substations FeatureServer)
 * then computes the nearest transmission line, the nearest substation
 * (tie-in / interconnect point), and counts inside the SCIP 0.25 / 0.5 / 1 mi
 * rings. Click any line for full segment details.
 *
 * Coordinates come from URL params so any SCIP page can deep-link here:
 *   /site-power-map?lat=28.3489&lon=-80.7419&label=Target%20A&address=...
 *   optional B/C:  &blat=..&blon=..&clat=..&clon=..   &ti=0
 * With no params it loads a Brevard County, FL demo site.
 */
...
// Line color by voltage (matches PowerLinesMap voltage step scale)
...
// Point→segment distance (miles) using a local equirectangular projection.
function pointToSegMiles(lat, lon, aLat, aLon, bLat, bLon) {
  const kx = Math.cos(toRad(lat)) * 69.172; // mi per deg lon
  const ky = 69.172;                          // mi per deg lat
...
function lineMinMiles(lat, lon, geom) {
  ...
  for (const ring of rings) {
    ...
    if (ring.length === 1) {
      const [x, y] = ring[0];
      const d = milesBetween(lat, lon, y, x);
      if (d < min) min = d;
    }
  }
  return min;
...
function circlePolygon(lat, lon, miles, steps = 72) {
  ...
  const dLon = miles / (Math.cos(toRad(lat)) * 69.172);
...
async function ensureMapbox() {
  ...
  if (!mapboxLoadingPromise) {
    mapboxLoadingPromise = new Promise((resolve, reject) => {
      ...
      css.rel = "stylesheet"; css.href = MAPBOX_CSS; document.head.appendChild(css);
      const s = document.createElement("script");
      s.src = MAPBOX_JS; s.onload = resolve; s.onerror = reject; document.head.appendChild(s);
    });
  }
...
async function fetchSubstations(lat, lon, miles) {
  ...
  const params = new URLSearchParams({
    ...
    outFields: "NAME,STATUS,TYPE,LINES,MAX_VOLT,MIN_VOLT,CITY,COUNTY",
...
export default function SitePowerMap() {
  ...
  const targets = useMemo(() => {
    ...
    if (Number.isFinite(lat) && Number.isFinite(lon)) {
      const out = [{
        label: params.get("label") || "Target A",
        lat, lon, address: params.get("address") || `${lat.toFixed(4)}, ${lon.toFixed(4)}`,
      }];
  ...
  const isDemo = targets === DEMO_TARGETS;
  const [activeIdx, setActiveIdx] = useState(() => {
    const ti = parseInt(params.get("ti"), 10);
    return Number.isFinite(ti) && ti >= 0 && ti < targets.length ? ti : 0;
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      map.on("load", () => {
        ...
        map.addLayer({ id: "rings-line", type: "line", source: "rings",
          paint: { "line-color": "#38bdf8", "line-width": 1.4, "line-opacity": 0.7, "line-dasharray": [2, 2] } });

        map.addSource("subconn", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
        map.addLayer({ id: "subconn-line", type: "line", source: "subconn",
          paint: { "line-color": "#facc15", "line-width": 2, "line-dasharray": [1.5, 1.2], "line-opacity": 0.9 } });

        map.addSource("lines", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
        map.addLayer({ id: "lines-halo", type: "line", source: "lines",
          paint: { "line-color": "#000", "line-width": ["interpolate", ["linear"], ["zoom"], 10, 3, 15, 7], "line-opacity": 0.35 } });
        map.addLayer({ id: "lines-color", type: "line", source: "lines",
          paint: { "line-color": LINE_COLOR_EXPR, "line-width": ["interpolate", ["linear"], ["zoom"], 10, 1.6, 15, 4], "line-opacity": 0.95 } });
        ...
        map.addLayer({ id: "subs-pt", type: "circle", source: "subs",
          paint: {
            "circle-radius": ["interpolate", ["linear"], ["zoom"], 10, 4, 15, 8],
            "circle-color": "#fde047", "circle-stroke-color": "#000", "circle-stroke-width": 1.2, "circle-opacity": 0.95,
          } });
        ...
        map.on("mousemove", "lines-hit", (e) => {
          ...
          hover.setLngLat(e.lngLat).setHTML(
            `<div style="font-family:monospace;font-size:11px">
               <strong>${voltLabel(p.VOLTAGE, p.VOLT_CLASS)}</strong> · ${p.OWNER ?? "N/A"}<br/>
               ${p.SUB_1 ?? "—"} → ${p.SUB_2 ?? "—"}</div>`
          ).addTo(map);
        });
        ...
        map.on("click", "lines-hit", (e) => {
          ...
          setSelected({ properties: f.properties, lngLat: [e.lngLat.lng, e.lngLat.lat] });
        });
        ...
        map.on("mousemove", "subs-pt", (e) => {
          ...
          subHover.setLngLat(e.lngLat).setHTML(
            `<div style="font-family:monospace;font-size:11px"><strong>⚡ ${p.NAME || "Substation"}</strong><br/>${mv}${p.STATUS ? " · " + p.STATUS : ""}</div>`
          ).addTo(map);
        });
  ...
  useEffect(() => {
    ...
    map.getSource("rings")?.setData({
      ...
      features: [0.25, 0.5, 1].map((mi) => circlePolygon(lat, lon, mi)),
    });
    ...
    (async () => {
      ...
      try {
        const [lineResp, subs] = await Promise.all([
        ...
        console.warn("SitePowerMap fetch failed:", err?.message);
      }
      ...
      // ---- compute surrounding-power stats ----
      ...
      for (const f of lineFeats) {
        ...
        if (d <= 1) {
          ...
          vclasses.add(voltLabel(f.properties?.VOLTAGE, f.properties?.VOLT_CLASS));
        }
        if (d < nearestLineDist) { nearestLineDist = d; nearestLine = f; }
      }

      let nearestSub = null, nearestSubDist = Infinity, subsWithin1 = 0, subsWithin5 = 0;
      for (const s of subFeats) {
        const c = s.geometry?.coordinates; if (!c) continue;
        const d = milesBetween(lat, lon, c[1], c[0]);
        if (d <= 1) subsWithin1++;
        if (d <= 5) subsWithin5++;
        if (d < nearestSubDist) { nearestSubDist = d; nearestSub = s; }
      }

      // dashed connector to nearest substation
      if (nearestSub?.geometry?.coordinates) {
        map.getSource("subconn")?.setData({
          type: "FeatureCollection",
          features: [{ type: "Feature", geometry: { type: "LineString", coordinates: [[lon, lat], nearestSub.geometry.coordinates] }, properties: {} }],
        });
      } else {
        map.getSource("subconn")?.setData({ type: "FeatureCollection", features: [] });
      }

      setStats({
        lineCount: lineFeats.length,
        rings,
        nearestLine: nearestLine ? { d: nearestLineDist, p: nearestLine.properties } : null,
        owners: [...owners],
        vclasses: [...vclasses],
        subCount: subFeats.length,
        subsWithin1, subsWithin5,
        nearestSub: nearestSub ? { d: nearestSubDist, p: nearestSub.properties } : null,
      });
      setLoading(false);
    })();
  }, [ready, target]);
    ...
    return (
      <div className="max-w-3xl mx-auto p-8">
        <div className="rounded-xl border border-amber-500/40 bg-amber-50 dark:bg-amber-950/20 p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-amber-800 dark:text-amber-200">Mapbox token unavailable — the Site Power Map can't render.</div>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="rounded-xl bg-gradient-to-r from-amber-500/15 via-transparent to-transparent border border-amber-500/30 px-5 py-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-0.5 truncate">
            {target.label} · {target.address}{isDemo && <span className="ml-2 text-amber-600 font-mono text-xs">DEMO SITE</span>}
          </p>
        </div>
        {/* Target A/B/C selector */}
        <div className="flex items-center gap-1.5">
          {targets.map((t, i) => (
            <button key={i} onClick={() => setActiveIdx(i)}
              className={`px-3 py-1.5 rounded-lg text-xs font-mono border transition ${
                i === activeIdx ? "bg-amber-500/20 border-amber-500/60 text-amber-700" : "bg-transparent border-border text-muted-foreground hover:text-foreground"
              }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-4">
        ...
        <div className="relative w-full h-[640px] rounded-lg overflow-hidden border border-border">
          ...
          {loading && (
            <div className="absolute top-3 left-3 bg-black/70 text-amber-300 text-[11px] font-mono px-2 py-1 rounded flex items-center gap-1.5">
              <Loader2 className="w-3 h-3 animate-spin" /> LOADING GRID…
            </div>
          )}
        ...
        {/* right column: surrounding-power stats + clicked details */}
        <div className="space-y-4">
          <div className="border border-border rounded-lg bg-card overflow-hidden">
            <div className="bg-gradient-to-r from-amber-600 to-yellow-600 text-white px-3 py-2 flex items-center gap-2">
              <Gauge className="w-4 h-4" />
              <span className="font-heading font-semibold text-sm">Surrounding Power Structure</span>
            </div>
            <div className="px-3 py-2">
              ...
              <div className="mt-3 mb-2">
                ...
                {ns ? (
                  <div className="bg-muted/50 rounded p-2">
                    <div className="text-sm font-bold">{ns.p.NAME || "Substation"}</div>
                    <div className="text-xs text-muted-foreground">
                      {ns.d.toFixed(2)} mi away{Number(ns.p.MAX_VOLT) > 0 ? ` · ${ns.p.MAX_VOLT} kV` : ""}{ns.p.LINES ? ` · ${ns.p.LINES} lines` : ""}
                    </div>
            ...
            <div className="px-3 py-1.5 border-t border-border bg-muted/30 text-[10px] font-mono text-muted-foreground tracking-wider flex items-center gap-1.5">
              <Radio className="w-3 h-3" /> SOURCE · HIFLD TRANSMISSION LINES + SUBSTATIONS
            </div>
          </div>
