 ...
 * strictest, often 2x the side) never applies. Guessing wrong in the permissive
 * direction is the dangerous failure, so when the roads are ambiguous this
 * returns default_side and says so rather than picking a frontage it cannot
 * defend.
 *
 * Corner lots are handled explicitly: a lot with street frontage on two sides
 * genuinely carries TWO front setbacks under most codes, and calling the second
...
function allSide(count: number, note: string, residential?: number[]): FrontageResult {
  const specs: EdgeSpec[] = Array.from({ length: count }, (_, i) => ({
...
/**
 * Type each parcel edge from nearby road centerlines.
 * Parcel and roads must already be in the same planar frame, in feet
 * (see projectParcelToFeet in HawkPerchSolver).
 */
export function typeParcelEdges(
  ...
  if (poly.length < 3) return allSide(poly.length, 'Parcel ring has fewer than three vertices — cannot type edges.', residential);

  const usableRoads = (roads || []).filter((r) => r?.coords?.length >= 2);
  if (!usableRoads.length) {
    return allSide(poly.length, 'No mapped roads found near this parcel — every line treated as a side line.', residential);
  }
  ...
  // that only touches their shared corner.
  ...
  if (!Number.isFinite(nearest) || nearest > maxDist) {
    return allSide(
      poly.length,
      `Nearest mapped road is ${Number.isFinite(nearest) ? Math.round(nearest) : '∞'} ft away — no direct frontage, so every line is treated as a side line. Verify access and frontage manually.`,
      residential
    );
  }
  ...
  // edge. Parallelism keeps a long flanking side line from being mistyped as the
  // rear on an irregular lot.
  ...
  const rearCandidates = measured.filter((m) => !frontIdx.includes(m.index) && isParallel(m.dir));
  const pool = rearCandidates.length ? rearCandidates : measured.filter((m) => !frontIdx.includes(m.index));
  ...
  const others = measured.filter((m) => !frontIdx.includes(m.index)).map((m) => m.dist);
  const gap = others.length ? Math.min(...others) - nearest : Infinity;
  const confidence: FrontageResult['confidence'] =
    nearest <= 120 && gap >= 60 ? 'high' : nearest <= maxDist && gap >= 20 ? 'medium' : 'low';
  ...
  const note =
    frontIdx.length > 1
      ...
      : `Frontage detected ${Math.round(nearest)} ft from${frontName ? ` ${frontName}` : ' the nearest mapped road'}.`;
  ...
  return {
    ...
    diagnostics: measured.map((m, i) => ({
      index: m.index,
      type: edgeSpecs[i].type,
      roadDistFt: m.dist,
      roadName: m.name,
      lengthFt: m.lengthFt,
    })),
    note:
      confidence === 'low'
        ? `${note} Confidence is low — confirm the frontage before relying on the front setback.`
        : note,
  };
}
