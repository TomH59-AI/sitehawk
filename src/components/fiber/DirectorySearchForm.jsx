...
export default function DirectorySearchForm({ zip, onZipChange, onSubmit, onClear, loading, hasResult }) {
  return (
    <form onSubmit={onSubmit} className="mt-5 flex gap-2">
      ...
      <button type="button" onClick={onClear} disabled={loading || (!zip && !hasResult)} className="inline-flex items-center gap-1 rounded-lg border border-border bg-card px-4 py-2 text-sm font-semibold text-foreground disabled:opacity-50">
